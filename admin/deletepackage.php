...
if(isset($_POST["sbmt"]))
{
	...
	$s="delete  from package where package_id='" . $_POST["pack_id"] . "'";
	mysqli_query($conn,$s);
	mysqli_close($conn);
...
<tr><td class="lefttxt">Select Package Name</td><td><select name="pack_id" required/><option value="">Select</option>
...
$s="select * from package where package_id = '" . $_POST["pack_id"] . "'";
$result=mysqli_query($conn,$s);
$r=mysqli_num_rows($result);
//echo $r;
...
while($data=mysqli_fetch_array($result))
{
	...
	$image = $data['package_image'];	
	


?>
<tr><td class="lefttxt"></td><td><img src="packimages/<?php echo $image; ?>" width="150px" height="50px" />
